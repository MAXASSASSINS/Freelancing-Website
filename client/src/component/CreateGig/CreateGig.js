...
export const CreateGig = () => {
  ...
  const [requirementsShortageWarning, setRequirementsShortageWarning] = useState(false);
  ...
  const [filteredCountries, setFilteredCountries] = useState(countryWithPhoneCodesData);
  ...
  const [showGigPublishFinalModal, setShowGigPublishFinalModal] = useState(false);

  const [showDataSendingLoadingScreen, setShowDataSendingLoadingScreen] = useState(false);
  ...
  useEffect(() => {
    if (gigDetail && gigId != 'null') {
      ...
      selectedSubCategoryRef.current.setChoosedOptionComingFromParent(subCategory);
      ...
      pricing.map((item, index) => {
        packageNameRefs.current[index].current.setTextComingFromParent(item.packageTitle);
        packageDescriptionRefs.current[index].current.setTextComingFromParent(item.packageDescription)
        deliveryTimeRefs.current[index].current.setChoosedOptionComingFromParent(item.packageDeliveryTime);
        deliveryRevisionsRefs.current[index].current.setChoosedOptionComingFromParent(item.revisions);
        packagePriceRefs.current[index].current.value = item.packagePrice;
        sourceFileRefs.current[index].current.setIsCheckedComingFromParent(item.sourceFile)
        commercialUseRefs.current[index].current.setIsCheckedComingFromParent(item.commercialUse);
      })
      ...
      images.map((item, index) => {
        sellerShowcaseImagesRefs.current[index].current.setFileComingFromParent(item);
      })
  ...
  const handleSendData = async () => {
    ...
    if (searchParams.get('id') === 'null') {
      ...
      console.log(data);
      const res = await axios.put(`/gig/update/${searchParams.get('id')}`, data);
  ...
  const checkForWarnings = () => {
    switch (currentStep) {
      ...
      case 5:
        ...
        if (sellerShowcaseImagesError || sellerShowcaseDocumentsError || sellerShowcaseVideoError) return true;
        break;
    }
  }
      ...
      case 2:
        ...
        packageNameRefs.current.forEach((item, index) => {
          const data = {
            ...
            packageDescription: packageDescriptionRefs.current[index].current.currValue,
            packageDeliveryTime: deliveryTimeRefs.current[index].current.currValue,
            revisions: Number(deliveryRevisionsRefs.current[index].current.currValue),
  ...
  useEffect(() => {
    let temp = countryWithPhoneCodesData.filter((item) =>
    ...
    <div className='create-gig-main'>
      <nav>
        ...
        <ul className='save-and-preview-wrapper noSelect'>
          ...
          <p>|</p>
          <li onClick={handleClickOnNavigationSaveAndPreview}>Save & Preview</li>
        </ul>
      </nav>
        ...
        <div className='overview-wrapper'>
          ...
          <div className='right-side'>
            ...
            <div className='keyword-section'>
              ...
              <p className='recommend'>5 tags maximum. Use letters and numbers only. <br />Tags should be comma seprated.</p>
              {
                tagListWarning &&
                <p className='tag-list-warning'>Tag list must contain at least 1 tag</p>
              }
            </div>
          </div>
        </div>
      ...
      <div className='pricing' style={{ display: currentStep === 2 ? "" : "none" }}>
        ...
        <div className='pricing-wrapper'>
          ...
          <div className='pricing-section'>
            ...
            {
              packagesWarning &&
              <p className='pricing-section-warning'><AiFillExclamationCircle></AiFillExclamationCircle> Please fill all the required fields</p>
            }
          </div>
        </div>
      ...
      <div className='description' style={{ display: currentStep === 3 ? "" : "none" }}>
        ...
        <div className='description-wrapper'>
          ...
          {
            gigDescriptionError &&
            <p className='gig-description-error'>Description should be at least 120 chars long.</p>
          }
        </div>
      </div>
      ...
      <div className='requirements' style={{ display: currentStep === 4 ? "" : "none" }}>
        ...
        <div className='requirements-wrapper'>
          ...
          }
          {
            ...
            <p className='requirements-shortage-warning'>You must add at least 1 requirement</p>
          }
        </div>
...

          {
            ...
            <div className='verify-phone-number-modal-wrapper'>
              <div className='verify-phone-number-modal'>
                <div className='close-icon' onClick={handleCloseVerifyPhoneNumberModal}>
                  <IoIosClose />
                </div>
...

                {
                  showVerifyCodeInput ?
                    <>
                      <p className='verfication-code-sent-notification'>A verification code has been sent to:</p>
                      ...
                      <div className='verification-code-wrapper'>
                        <p className='enter-verification-code-message'>Please enter the Verification code</p>
                        <input
                          type="text"
                          inputMode='numeric'
                          maxLength={6}
                        ...
                        />
                        <p ref={verificationCodeErrorRef} className='invalid-code-message'>Oops... that code is wrong..<br />Please verify your code and try again.</p>
                        <button
                          className={verificationCode.length === 6 ? 'active' : ''}
                      ...
                      <p className='verification-code-not-receive-message'>
                        ...
                        <span onClick={() => { setShowVerifyCodeInput(false); setVerificationCode("") }}
                          onclick={handleGoBackToEnterPhoneNumber}
                        > Back</span>
                        , check that you have entered the right number, and try again
                      </p>

                    </>
                    :
                    <>
                      <p className='thanks-para'>Thank you for taking a moment to verify your phone number. </p>
                      ...
                      <div className='modal-inputs-wrapper'>
                        <div className='modal-input'>
                          ...
                          />
                          {
                            showCountryDropdown && country && filteredCountries.length > 0 &&
                            <ul className='country-code-wrapper'>
                              {
                                filteredCountries.map((countryData, index) => {
                                  return (
                                    <li className='country-code' onClick={() => handleClickOnCountryListItem(countryData)}>
                        ...
                        <div className='modal-input'>
                          ...
                          {
                            dialCode &&
                            <p className='dial-code'>{dialCode} - </p>
                          }
                          <input
                            type="tel"
                            value={phoneNumber}
                        ...
                        <div ref={invalidPhoneNumberRef} className='wrong-phone-number-warning'>Please enter a valid number</div>
                      </div>

                      <div className='verify-buttons'>
                        <button
                          className={phoneNumber.length >= 5 && country && dialCode ? "button-active" : ""}
