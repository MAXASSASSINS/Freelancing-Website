...
export const Chat = ({ gigDetail, showChatBox, setShowChatBox }) => {
  ...
  const getAllMessagesForSingleUser = async () => {
    ...
    const { data } = await axios.get('/get/all/messages/for/current/user', config);
    console.log(data);
  }
            ...
            (!allMessages || checkUserOpenItsOwnGig()) ?
              (
                <div className='chat-suggestion-div'>
                  <ul>
                    <li ref={suggestionRef1} onClick={(e) => handleChatSuggestion(e)}>&#128075; Hey! {gigDetail.user.name}, can you help me with...</li>
                    <li ref={suggestionRef2} onClick={(e) => handleChatSuggestion(e)}>Do you have any experience with...</li>
                    <li ref={suggestionRef3} onClick={(e) => handleChatSuggestion(e)}>Do you think you can deliver your order by...</li>
                  </ul>
                </div>
              )
              :
                        ...
                        <li key={message._id}>
                          {
                            message.sender.avatar.url ?
                              ...
                              :
                              <i className={"fa-solid fa-" + (message.sender.name[0].toLowerCase())}></i>
                          }
                          <div>
                            <div className='chat-message-owner-time-div'>
                              <span className='chat-message-owner'>{message.sender._id === user._id ? "Me" : message.sender.name}</span>
                              &nbsp;
                              <span className='chat-message-time'><Moment format='D MMM,  H:mm' >{message.updatedAt}</Moment></span>
                            </div>
          ...
          <form id='chat-form' onSubmit={(e) => sendChat(e)}>
            {
              ...
              <div className='chat-attached-files-div'>
                <div className='chat-attached-files-heading'>ATTACHED FILES ({selectedFiles.length})</div>
                <ul>
                  {
